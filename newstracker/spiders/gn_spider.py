...
class GnSpider(scrapy.Spider):
    ...
    # company_name = raw_input("What company or subject or country would you like to analyze? : ")
        ...
        for news in newss:
            ...
            if my_title.extract():
                if my_title.re(company_name):
                    ...
                    cmdping = "curl -d 'text=" + my_title.extract()[0].encode('utf-8').replace("'","") + "' http://text-processing.com/api/sentiment/"
                    # print cmdping
                    p = subprocess.Popen(cmdping, stdout=subprocess.PIPE, shell=True)
                    (output, err) = p.communicate()
                    # print output.split()[-1][1:-2]
            ...
            if my_desc.extract():
                ...
                cmdping = "curl -d 'text=" + my_desc.extract()[0].encode('utf-8').replace("'","") + "' http://text-processing.com/api/sentiment/"
                # print cmdping
